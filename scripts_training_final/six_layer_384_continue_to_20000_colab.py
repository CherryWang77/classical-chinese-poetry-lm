...
# =========================
# Colab / Drive project root
# =========================
PROJECT_ROOT = Path("/content/drive/MyDrive/lab3_real_experiment")

# =========================
# Files and output paths
# =========================
INPUT_FILE = PROJECT_ROOT / "songci_main_5mb.txt"
CHECKPOINT_TO_LOAD = PROJECT_ROOT / "checkpoints_six_layer_384" / "six_layer_384_step_5000.pt"

OUTPUT_CSV = PROJECT_ROOT / "six_layer_384_metrics_continued_5001_to_20000.csv"
CHECKPOINT_DIR = PROJECT_ROOT / "checkpoints_six_layer_384"
SAMPLES_DIR = PROJECT_ROOT / "samples_six_layer_384"
...
def main():
    ...
    if start_step >= TARGET_STEP:
        raise ValueError(f"Checkpoint step {start_step} is already >= target step {TARGET_STEP}")
    ...
    optimizer = torch.optim.AdamW(model.parameters(), lr=ckpt_config["learning_rate"])

    model.load_state_dict(checkpoint["model_state_dict"])
    optimizer.load_state_dict(checkpoint["optimizer_state_dict"])

    print("=== Continue 6-layer + 384-dim training to 20000 (Colab) ===")
    ...
    print("Context length:", ckpt_config["context_length"])
    print("Batch size:", ckpt_config["batch_size"])
    print("Embed dim:", ckpt_config["embed_dim"])
    print("Num layers:", ckpt_config["num_layers"])
    print("Num heads:", ckpt_config["num_heads"])
    print("Dropout:", ckpt_config["dropout"])
    print("Learning rate:", ckpt_config["learning_rate"])
    ...
    for step in range(start_step + 1, TARGET_STEP + 1):
        _, x_train, y_train = get_batch_as_tensors(
            train_ids, ckpt_config["batch_size"], ckpt_config["context_length"], device
        )
        ...
        if step % EVAL_EVERY == 0:
            avg_val_loss, _ = evaluate_average_val_loss(
                model,
                val_ids,
                ckpt_config["batch_size"],
                ckpt_config["context_length"],
                NUM_VAL_BATCHES,
                device
            )
        ...
        if step % SAVE_EVERY == 0:
            ckpt_path = CHECKPOINT_DIR / f"six_layer_384_step_{step}.pt"
            save_checkpoint(ckpt_path, model, optimizer, step, ckpt_config)
        ...
        if step % GENERATE_EVERY == 0:
            ...
            sample_path = SAMPLES_DIR / f"six_layer_384_sample_step_{step}.txt"
            save_sample_text(sample_path, sample_text)
            print("Sample saved to:", sample_path)

    final_ckpt_path = CHECKPOINT_DIR / "six_layer_384_final_step_20000.pt"
    save_checkpoint(final_ckpt_path, model, optimizer, TARGET_STEP, ckpt_config)

    write_csv_log(csv_rows, OUTPUT_CSV)

    print("\n6-layer + 384-dim continuation to 20000 completed.")
