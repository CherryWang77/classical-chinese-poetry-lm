...
# Output CSV log
OUTPUT_CSV = Path("decoder_block_50steps_metrics.csv")

# Checkpoint directory
CHECKPOINT_DIR = Path("checkpoints_decoder_block")
...
def main():
    ...
    print("=== Decoder block train + avg validation + BPC loop (50 steps, checkpoints) ===")
    ...
    for step in range(1, NUM_STEPS + 1):
        ...
        if step % SAVE_EVERY == 0:
            ckpt_path = CHECKPOINT_DIR / f"decoder_block_step_{step}.pt"
            save_checkpoint(ckpt_path, model, optimizer, step, vocab_size)
            print("Checkpoint saved to:", ckpt_path)

    final_ckpt_path = CHECKPOINT_DIR / "decoder_block_final.pt"
