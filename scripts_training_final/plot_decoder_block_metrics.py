...
# Input CSV
INPUT_CSV = Path("decoder_block_50steps_metrics.csv")

# Output plots
LOSS_PLOT = Path("decoder_block_loss_plot.png")
BPC_PLOT = Path("decoder_block_bpc_plot.png")

def main():
    # 1. Read CSV
    df = pd.read_csv(INPUT_CSV)

    print("=== Plot decoder block metrics ===")
    ...
    # 3. Print a quick preview
    print("\nFirst 10 rows:")
    print(df.head(10).to_string(index=False))

    # 4. Validation-only subset
    ...
    plt.xlabel("Step")
    plt.ylabel("Loss")
    plt.title("Decoder block training and averaged validation loss")
    plt.legend()
    plt.tight_layout()
    ...
    plt.xlabel("Step")
    plt.ylabel("BPC")
    plt.title("Decoder block training and averaged validation BPC")
    plt.legend()
    plt.tight_layout()
